...
const isIcsCategory = id && String(id).toLowerCase().trim() === 'ics';

  useEffect(() => {
    const fetchComponents = async () => {
      // If we are on the ICs root, do not fetch components yet.
      if (isIcsCategory && !selectedSubCategory) {
        setComponentList([]);
        setLoading(false);
        return;
      }
  ...
  const isICRoot = isIcsCategory && !selectedSubCategory;
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle} numberOfLines={1}>
          {displayTitle} [{String(id)}|{String(isIcsCategory)}]
        </Text>
        <View style={{ width: 28 }} /> 
      </View>
